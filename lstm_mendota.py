...
def param_optimizer(input_parameter,Hyper_params,data):
    ...
    for k in trange(input_parameter['num']):
        ...
        #
        train_X, train_Y = torch.as_tensor(normaler.transform(train_X), dtype=torch.float).to(device),\
                           torch.as_tensor(train_Y, dtype=torch.float).to(device)
        test_X,test_Y = torch.as_tensor(normaler.transform(test_X), dtype=torch.float).to(device),\
...
def model_prediction(input_parameter=None,optim_keeper=None,data=None):
    ...
    for k in trange(input_parameter['num']):
        ...
        train_X, train_Y = torch.as_tensor(normaler.transform(train_X), dtype=torch.float).to(device),\
                           torch.as_tensor(train_Y, dtype=torch.float).to(device)
        clib_X,pred_X = torch.as_tensor(normaler.transform(clib_X), dtype=torch.float).to(device),\
                        torch.as_tensor(normaler.transform(pred_X), dtype=torch.float).to(device)
        #
        train_loader = Data.DataLoader(Data.TensorDataset(train_X, train_Y), batch_size=batch_size,
                                   shuffle=True, num_workers=0,worker_init_fn=seed_worker,generator=g)
...
if __name__ == '__main__':
    ...
    input_parameter=dict(
        ...
        no_cuda=False,
        resolution='month'  # day, hour, month
    )
    #  data
    ...
    ##---------------- prediction phase-------------- ##
    optims = pd.read_excel("~/optimal_hyperparameter/lstm_mendota_optim_keeper.xlsx",dtype=object)
